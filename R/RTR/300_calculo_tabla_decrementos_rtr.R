...
message( '\tCargando las tablas ajustadas de la mortalidad y tasa de salida de RTR' )
...
load( paste0( parametros$RData_seg, 'IESS_RTR_transicion_activo_jubilado_int.RData' ) )
...
ud_smooth_model_f <- lm( log_ud_est ~ bs( x, df = 3, degree = 3, 
                                          Boundary.knots = range( tab_dec_proj_f$x ) ), 
                         data = tab_dec[ sexo == 'F' & is.finite( log_ud_est ) & 
                                           x >= 20 & x <= 85 ] )
tab_dec_proj_f[ , log_ud := predict( object = ud_smooth_model_f,
                                     newdata = tab_dec_proj_f ) ]

ud_smooth_model_m <- lm( log_ud_est ~ bs( x, df = 7, degree = 3, 
                                          Boundary.knots = range( tab_dec_proj_m$x ) ),
                         data = tab_dec[ sexo == 'M' & is.finite( log_ud_est ) & 
                                           x >= 20 & x <= 89  ] )
tab_dec_proj_m[ , log_ud := predict( object = ud_smooth_model_m, 
                                     newdata = tab_dec_proj_m ) ]

# Alisando retiro por vejez IVM---------------------------------------------------------------------
uv_smooth_model_f <- lm( log_uv_est ~ bs( x, df = 8, degree = 3, 
                                          Boundary.knots = range( tab_dec_proj_f$x ) ), 
...
uv_smooth_model_m <- lm( log_uv_est ~ bs( x, df = 8, degree = 3, 
                                          Boundary.knots = range( tab_dec_proj_m$x ) ), 
...
# # Alisando retiro por invalidez IVM---------------------------------------------------------------
# ui_smooth_model_f <- lm( log_ui_est ~ bs( x, df = 8, degree = 3, 
#                                           Boundary.knots = range( tab_dec_proj_f$x ) ),
...
#                                      newdata = tab_dec_proj_f[ , list( x ) ] ) ]
# 
# ui_smooth_model_m <- lm( log_ui_est ~ bs( x, df = 8, degree = 3, 
#                                           Boundary.knots = range( tab_dec_proj_m$x ) ),
...
# Tasa de retiro de invalidez de IVM----------------------------------------------------------------
tab_dec_proj_f[ , sexo := 'F' ]
tab_dec_proj_f<-merge( tab_dec_proj_f, 
                       trans_activo_invalidez[ sexo == 'F', 
                                               list( x, log_ui_est = log(uin),
                                                                  log_ui = loguin_int ) ], 
                       by = c( 'x' ), all.x = TRUE )

tab_dec_proj_m[ , sexo := 'M' ]
tab_dec_proj_m<-merge( tab_dec_proj_m, 
                       trans_activo_invalidez[ sexo == 'M', 
                                               list( x, log_ui_est = log(uin), 
                                                                  log_ui = loguin_int ) ], 
                       by = c( 'x' ), all.x = TRUE )


#Tasa de activos a jubilado por incapacidad permanente, total y absoluta----------------------------
tab_dec_proj_f[ , sexo := 'F' ]
tab_dec_proj_f<-merge( tab_dec_proj_f, 
                       tasa_jubilacion_rtr_int[,list(x,
                                                     uin_est = ui_rtr,
                                                     log_uin_est = log_ui_rtr,
                                                     log_uin = log_ui_rtr_int)],
                       by = c( 'x' ), all.x = TRUE )
...
#Tasa de mortalidad de jubilados por incapacidad permanente total, absoluta y parcial---------------
tab_dec_proj_f[ , sexo := 'F' ]
tab_dec_proj_f<-merge( tab_dec_proj_f, 
                       mortalidad_jubilados_rtr_int[,list(x,
                                                     udin_est = ud_rtr,
                                                     log_duin_est = log_ud_rtr,
                                                     log_duin = log_ud_rtr_int)],
                       by = c( 'x' ), all.x = TRUE )
...
tab_dec_proj_m<-merge( tab_dec_proj_m, 
                       mortalidad_jubilados_rtr_int[,list(x,
                                                     ...
                                                     log_duin = log_ud_rtr_int)],
                       by = c( 'x' ), all.x = TRUE )
#Agrupar tasas de decrementos-----------------------------------------------------------------------
aux <- rbind( tab_dec_proj_f, tab_dec_proj_m )
tab_dec <- merge( tab_dec, aux, by = c( 'sexo', 'x' ), all.y = TRUE )

tab_dec <- tab_dec[ , list( sexo, x, ER, 
                            Nd, Ni, Nr, 
                            ud_est, ui_est, uv_est, uin_est, udin_est,
                            log_ud_est, log_ui_est, log_uv_est, log_uin_est, log_duin_est,
                            log_ud, log_ui, log_uv, log_uin, log_duin) ]
setorder( tab_dec, sexo, x )

#Calculando la tabla de decrementos de RTR----------------------------------------------------------
...
tab_dec[ , uin:= exp( log_uin ) ]
tab_dec[ , udin:= exp( log_duin ) ]
tab_dec[ x <= 48, uv := 0 ]
tab_dec[ x >= 100, uv := 0 ]
tab_dec[ , u_tau := ud + ui + uv + uin + udin]
...
tab_dec[ is.finite( uin ), qin_ind := 1 - exp( -uin ) ]
tab_dec[ !is.finite( uin ), qin_ind := 1 ]
tab_dec[ is.finite( udin ), qdin_ind := 1 - exp( -udin ) ]
tab_dec[ !is.finite( udin ), qdin_ind := 1 ]
tab_dec[ , pd_ind := 1 - qd_ind ]
tab_dec[ , pi_ind := 1 - qi_ind ]
tab_dec[ , pv_ind := 1 - qv_ind ]
tab_dec[ , pin_ind := 1 - qin_ind ]
tab_dec[ , pdin_ind := 1 - qdin_ind ]

# Cálculo probabilidades totales -------------------------------------------------------------------
tab_dec[ , p_tau := pd_ind * pi_ind * pv_ind * pin_ind * pdin_ind]
tab_dec[ , q_tau := 1 - p_tau ]

# Cálculo probabilidades dependientes --------------------------------------------------------------
# suponiendo fuerzas constantes de decrementos
...
tab_dec[ is.finite( u_tau ), qin := uin * q_tau / u_tau ]
tab_dec[ !is.finite( u_tau ), qin := 0 ]
tab_dec[ is.finite( u_tau ), qdin := udin * q_tau / u_tau ]
tab_dec[ !is.finite( u_tau ), qdin := 0 ]
tab_dec[ , pd := 1 - qd ]
tab_dec[ , pi := 1 - qi ]
tab_dec[ , pv := 1 - qv ]
tab_dec[ , pin := 1 - qin ]
...
tab_dec[ , ein := cumprod( pin ), by = list( sexo ) ]
tab_dec[ , ein := rev( ein ), by = list( sexo ) ]
tab_dec[ , ein := cumsum( ein ), by = list( sexo ) ]
tab_dec[ , ein := rev( ein ), by = list( sexo ) ]
tab_dec[ , ein := ein + 0.5 ]

tab_dec[ , edin := cumprod( pdin ), by = list( sexo ) ]
tab_dec[ , edin := rev( edin ), by = list( sexo ) ]
tab_dec[ , edin := cumsum( edin ), by = list( sexo ) ]
tab_dec[ , edin := rev( edin ), by = list( sexo ) ]
tab_dec[ , edin := edin + 0.5 ]
